...
/** Public PC / Mobile preview (로그인 없음) */
export const COVERAGE_SIMULATOR_PREVIEW_PC_STORAGE_KEY = 'coverage-simulator-preview-pc:v1'
export const COVERAGE_SIMULATOR_PREVIEW_MOBILE_STORAGE_KEY = 'coverage-simulator-preview-mobile:v1'
...
function storageKey(userKey: string): string {
  if (userKey === COVERAGE_SIMULATOR_PREVIEW_PC_USER_KEY) {
    return COVERAGE_SIMULATOR_PREVIEW_PC_STORAGE_KEY
  }
  if (userKey === COVERAGE_SIMULATOR_PREVIEW_MOBILE_USER_KEY) {
    return COVERAGE_SIMULATOR_PREVIEW_MOBILE_STORAGE_KEY
  }
