...
import { resolveViteNaverMapClientId } from './server/lib/resolveViteNaverMapClientId.js'
...
export default defineConfig(({ mode }) => {
  ...
  const viteNaverMapClientId = resolveViteNaverMapClientId(env)

  return {
    define: {
      __INSURANCE_WEB_APP_VERSION__: JSON.stringify(insurancePkg.version),
      __INSURANCE_WEB_BUILD_ID__: JSON.stringify(WEB_BUILD_ID),
      'import.meta.env.VITE_NAVER_MAP_CLIENT_ID': JSON.stringify(viteNaverMapClientId),
    },
