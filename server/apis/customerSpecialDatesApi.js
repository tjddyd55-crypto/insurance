...
export function registerCustomerSpecialDatesApi(apiRouter, { pool, requireAuth, handleDbError }) {
  ...
  apiRouter.post('/customers/:customerId/special-dates', requireAuth, async (req, res) => {
    ...
    if (!title) {
      res.status(400).json({ message: `${CUSTOMER_DESIGNATED_DATE_LABEL} 이름을 입력해 주세요.` })
      return
    }
    if (!dateValue) {
      res.status(400).json({ message: `${CUSTOMER_DESIGNATED_DATE_LABEL} 날짜는 YYYY-MM-DD 형식이어야 합니다.` })
      return
    }
  ...
  apiRouter.patch('/customers/:customerId/special-dates/:specialDateId', requireAuth, async (req, res) => {
    ...
    try {
      ...
      if (cur.rowCount === 0) {
        await client.query('ROLLBACK')
        res.status(404).json({ message: `${CUSTOMER_DESIGNATED_DATE_LABEL}을 찾을 수 없습니다.` })
        return
      }
      ...
      if (Object.prototype.hasOwnProperty.call(body, 'purposeType')) {
        ...
        if (!next) {
          await client.query('ROLLBACK')
          res.status(400).json({ message: `유효한 ${CUSTOMER_DESIGNATED_DATE_LABEL} 타입이 아닙니다.` })
          return
        }
        purposeType = next
      }
