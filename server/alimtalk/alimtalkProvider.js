...
export function pickAligoAlimtalkCode(raw) {
  ...
  const code = obj.code ?? obj.result_code ?? obj.resultCode
...
export function pickAligoAlimtalkMessage(raw) {
  if (!raw || typeof raw !== 'object') return null
  const obj = /** @type {Record<string, unknown>} */ (raw)
  const msg = obj.message ?? obj.msg ?? obj.result_message
...
export function pickAligoAlimtalkMessageId(raw) {
  ...
  const id = (info && info.mid) || obj.mid || obj.message_id || obj.msg_id
...
export async function sendAligoAlimtalk(input) {
  ...
  try {
    ...
    const providerCode = pickAligoAlimtalkCode(parsed)
    const providerMessage = pickAligoAlimtalkMessage(parsed) || 'aligo response'
    const providerMessageId = pickAligoAlimtalkMessageId(parsed)
