...
describe('customer relation groups helpers', () => {
  ...
  it('listActiveGroupMembers SQL includes ga_id tenant scope', async () => {
    ...
    assert.match(slice, /c\.ssn\s+AS\s+customer_ssn/)
    assert.match(slice, /c\.phone\s+AS\s+customer_phone/)
    assert.match(slice, /resolveCustomerBirthDateYmd/)
    // 응답 객체에 ssn 필드를 노출하지 않는다
    const returnBlock = slice.slice(slice.indexOf('return {'), slice.indexOf('sortOrder') + 80)
    assert.equal(returnBlock.includes('ssn:'), false)
    assert.match(returnBlock, /birthDate:/)
  })
})
